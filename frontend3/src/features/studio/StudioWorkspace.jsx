...
function StudioWorkspace({ onLogout }) {
  ...
  return (
    <div className="app-root">
      <header className="deck-main-topbar">
        <button
          type="button"
          className="deck-logo"
        ...
        <div className="deck-main-seg">
          <span className="deck-seg-label">Mode</span>
          <span className="deck-seg-value">Creator Console</span>
        </div>
        <nav className="deck-main-tabs" aria-label="Main tabs">
          <a
            href="#dashboard"
            className={mainTab === "dashboard" ? "deck-main-tab active" : "deck-main-tab"}
            onClick={(event) => {
          ...
          <a
            href="#library"
            className={mainTab === "library" ? "deck-main-tab active" : "deck-main-tab"}
            onClick={(event) => {
              event.preventDefault();
              setMainTab("library");
            }}
  ...
  const quickStartActions = [
    {
      ...
      number: "01 /",
      kicker: "Create",
      title: "Record a routine",
      description: "Capture your dance from webcam with music, then auto-generate a routine package you can share.",
      footnote: "Best for making original choreography.",
      onClick: onRecord,
    },
    {
      id: "browse",
      ...
      description: "Load your saved sessions, continue practice runs, and compare against routines you already imported.",
      footnote: "Best for improving existing routines.",
      onClick: onBrowse,
    },
    {
      id: "friend-code",
      ...
      description: "Import a friend’s shared routine package and jump straight into a play session with scoring.",
      footnote: "Best for challenges and leaderboard runs.",
      onClick: onFriendCode,
    },
  ];
  return (
    <div className="dashboard-stack deck-dashboard">
      <section className="deck-hero">
        <p className="deck-hero-eyebrow">LEARN TO DANCE | CREATOR CONSOLE</p>
        <h2 className="deck-hero-title">BROWSER<br />BOOGIE</h2>
        <p className="deck-hero-sub">
          Build routines, share codes, and compete on scored practice runs.
        </p>
      </section>
      ...
      <section className="dashboard-clean">
        <div className="deck-section-head">
          ...
          <p className="muted">Pick one mode and jump straight into studio.</p>
        </div>
        <div className="dash-actions">
          {quickStartActions.map((action) => (
            <button key={action.id} className="dash-big-btn" onClick={action.onClick}>
              <span className="deck-card-num">{action.number}</span>
              <span className="dash-action-kicker">{action.kicker}</span>
              <span className="dash-action-title">{action.title}</span>
              <span className="dash-action-text">{action.description}</span>
              <span className="dash-action-foot">{action.footnote}</span>
              <span className="deck-card-arrow">→ Open</span>
            </button>
          ))}
        </div>
      </section>
        ...
        {!statsLoading && !statsError && hasStats && (
          <div className="stats-grid">
            ...
            <article className="stats-item">
              ...
              <h3>{Number(userStats.bestScore || 0).toFixed(1)}%</h3>
            </article>
            <article className="stats-item">
            ...
            <div className="leaderboard-list" role="table" aria-label="Leaderboard">
              ...
              {leaderboardRows.map((row) => (
                ...
                >
                  ...
                  <span role="cell">{Number(row.bestScore || 0).toFixed(1)}%</span>
                  <span role="cell">{row.grade || scoreToLetterGrade(row.averageScore)}</span>
                </div>
              ))}
            </div>
          )}
...
function Library({ sessions, onOpenSession, onDeleteSession, onShareSession, onRecord, onImport }) {
  return (
    <section className="library-wrap deck-library">
      <div className="section-head">
        <div>
          <h2>My Library</h2>
          <p className="muted">Stored sessions, imports, and share-ready routines.</p>
        </div>
      ...
      <div className="library-list">
        {sessions.map((session, index) => (
          <article key={session.id} className="session-card deck-session-card">
            <div>
              <p className="deck-session-num">{String(index + 1).padStart(2, "0")} /</p>
              <h3>{session.title}</h3>
...
function StudioPage({
  ...
  if (!session) {
    return (
      <div className="app-root">
        <header className="deck-studio-topbar">
          <button type="button" className="deck-logo" onClick={onBack}>Browser Boogie</button>
          <div className="deck-studio-seg">
            <span className="deck-seg-label">Session</span>
            <span className="deck-seg-value">No Session Selected</span>
          </div>
          <div className="deck-studio-right">
  ...
  return (
    <div className={appRootClassName}>
      <header className="deck-studio-topbar">
        <button type="button" className="deck-logo" onClick={onGoDashboard}>
          Browser Boogie
        </button>
        <div className="deck-studio-seg">
          <span className="deck-seg-label">Session</span>
          <span className="deck-seg-value">{session.title}</span>
        </div>
        <div className="deck-studio-seg">
          <span className="deck-seg-label">Mode</span>
          <span className="deck-seg-value">{shownModeLabel}</span>
        </div>
        <div className="deck-studio-right">
          <button className="btn" onClick={onGoDashboard}>Dashboard</button>
          <button className="btn" onClick={onBack}>Back To Library</button>
          {typeof onLogout === "function" && (
            <button className="btn" onClick={onLogout}>Log Out</button>
          )}
      ...
      <div className="deck-studio-statusbar">
        <span className="deck-status-item">
          Camera:
          <b>{cameraState === "live" ? "Live" : cameraState === "requesting" ? "Requesting" : "Unavailable"}</b>
        </span>
        <span className="deck-status-item">
          Pose:
          <b>{poseState === "ready" ? "Tracking" : poseState === "loading" ? "Loading" : "Unavailable"}</b>
        </span>
        <span className="deck-status-item deck-status-message">{statusMessage || "Ready."}</span>
      </div>
