...
let sharedImportPoseLandmarkerPromise = null;

async function getSharedImportPoseLandmarker() {
  if (sharedImportPoseLandmarkerPromise) return sharedImportPoseLandmarkerPromise;
  sharedImportPoseLandmarkerPromise = (async () => {
  ...
  return sharedImportPoseLandmarkerPromise;
}
...
async function buildRoutineFromVideoFile(videoFile, routineName, onProgress) {
  const landmarker = await getSharedImportPoseLandmarker();
  ...
  try {
    ...
    for (let i = 0; i < totalSamples; i += 1) {
      ...
      const result = landmarker.detectForVideo(video, Math.round(tSec * 1000));
  ...
  return (
    <div className="shell">
      <header className="topbar">
        <div className="brand">
          <p className="eyebrow">JustDance Studio</p>
...
function StudioPage({ session, sessionFiles, onBack, onGoDashboard, onUpdateSession, onUpdateSessionFiles }) {
  ...
  function togglePlayLayout() {
    ...
    if (!referenceVideoUrl) {
      setStatusMessage("Side By Side needs a webcam reference video.");
      return;
    }
  ...
  const studioGridClass = [
    ...
    .filter(Boolean)
    .join(" ");
  const shellClassName = isPlayUiMode || isRecordUiMode ? "shell shell-play" : "shell";
              ...
              {isPlayUiMode && (
                <>
                  ...
                  <div className="floating-play-dock" role="group" aria-label="Play controls">
                    ...
                    <button className="btn" onClick={togglePlayLayout}>
